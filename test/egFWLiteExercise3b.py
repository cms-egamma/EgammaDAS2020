...
if __name__ == "__main__":
    ...
    for event_nr,event in enumerate(events):
        ...
        for ele in evtdata.get("eles"):
            ...
            if ele.et()>25 and abs(ele.superCluster().eta())<1.4442 and ele.electronID("cutBasedElectronID-Fall17-94X-V2-tight"):
            ...
            if (ele.et()>25 and abs(ele.superCluster().eta())<1.4442 and 
                (ele.userInt("cutBasedElectronID-Fall17-94X-V2-tight") & 0x3EF) == 0x3EF):
                is_data = event.object().event().isRealData()
                if is_data or GenTools.match_to_gen(ele.eta(),ele.phi(),evtdata.get("gen"))[0]:
                    sigmaIEtaIEtaSigN1Hist.Fill(ele.full5x5_sigmaIetaIeta())
                    etSigN1Hist.Fill(ele.et())
                #now fill for MC which has not been gen matched 
                elif not is_data: 
                    sigmaIEtaIEtaBkgN1Hist.Fill(ele.full5x5_sigmaIetaIeta())
                    etBkgN1Hist.Fill(ele.et())
